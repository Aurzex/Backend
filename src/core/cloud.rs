...
mod tests {
    ...
    #[test]
    fn cloud_value_serialization() {
        let n = CloudValue::Number(42);
        let s = CloudValue::Text("你好".into());
        assert_eq!(serde_json::to_string(&n).unwrap(), "42");
        assert_eq!(serde_json::to_string(&s).unwrap(), "\"你好\"");
        assert_eq!(n.param_type(), "number");
        assert_eq!(s.param_type(), "string");
    }

    #[test]
    fn cloud_value_from_json_conversions() {
        assert_eq!(CloudValue::from_json(&json!(7)), CloudValue::Number(7));
        assert_eq!(
            CloudValue::from_json(&json!("abc")),
            CloudValue::Text("abc".into())
        );
        assert_eq!(CloudValue::from_json(&json!(true)), CloudValue::Number(1));
        assert_eq!(
            CloudValue::from_json(&json!({"a": 1})),
            CloudValue::Text("{\"a\":1}".into())
        );
    }

    #[test]
    fn command_factory_private_public() {
        let value = CloudValue::Number(3);
    ...
    #[test]
    fn merge_commands_groups_and_dedups() {
        let v = CloudValue::Number(1);
        let commands = vec![
            CommandFactory::update_private_variable("a", &v),
            CommandFactory::update_private_variable("b", &v),
            CommandFactory::update_public_variable("c", &v),
            CommandFactory::update_list("L1", vec![json!({"action": "append", "value": 1})]),
            CommandFactory::update_list("L1", vec![json!({"action": "append", "value": 2})]),
            CommandFactory::update_list("L2", vec![json!({"action": "clear"})]),
        ];
        let merged = merge_commands(commands);
    ...
    fn parse_frame_variants() {
        ...
        assert_eq!(parse_frame("3"), Frame::Pong);
        assert_eq!(parse_frame("40"), Frame::Connected);
        assert_eq!(parse_frame("41"), Frame::ServerClose);
        assert_eq!(
            parse_frame("0{\"sid\":\"x\",\"pingInterval\":25000}"),
            Frame::Handshake(json!({"sid": "x", "pingInterval": 25000}))
        );
        assert_eq!(
            parse_frame("42[\"join\",\"123\"]"),
            Frame::Event("join".into(), json!(123))
        );
        assert_eq!(
            parse_frame("42[\"update_vars\",[{\"cvid\":\"1\",\"value\":2}]]"),
            Frame::Event("update_vars".into(), json!([{"cvid": "1", "value": 2}]))
        );
        ...
        // 无法解析的字符串载荷保持原样
        assert_eq!(
            parse_frame("42[\"update_vars_done\",\"fail\"]"),
            Frame::Event("update_vars_done".into(), json!("fail"))
        );
        assert_eq!(parse_frame("hello"), Frame::Unknown("hello".into()));
    }

    #[test]
    fn send_event_frame_shape() {
        let conn = CloudBuilder::new(12345).build();
        // 未连接时发送应返回 NotConnected
        assert!(matches!(
            conn.send_event("join", &json!("12345")),
            Err(CloudError::NotConnected)
        ));
    }

    #[test]
    fn execute_list_actions_semantics() {
        ...
        let out = execute_list_action(&mut items, &ListAction::Unshift(0i64.into())).unwrap();
        assert_eq!(out.op, "unshift");
        assert_eq!(items.first(), Some(&CloudValue::Number(0)));

        let out = execute_list_action(&mut items, &ListAction::DeleteAt(0)).unwrap();
        assert_eq!(out.op, "remove");
        assert_eq!(items.len(), 3);

        let out = execute_list_action(&mut items, &ListAction::ReplaceAt(1, 9i64.into())).unwrap();
        assert_eq!(out.op, "replace");
        assert_eq!(items[1], CloudValue::Number(9));

        let out = execute_list_action(&mut items, &ListAction::ReplaceLast(7i64.into())).unwrap();
        assert_eq!(out.op, "replace_last");
        assert_eq!(items.last(), Some(&CloudValue::Number(7)));

        let out = execute_list_action(&mut items, &ListAction::Insert(1, 5i64.into())).unwrap();
        assert_eq!(out.op, "insert");
        assert_eq!(items[1], CloudValue::Number(5));

        let out = execute_list_action(&mut items, &ListAction::DeleteAll).unwrap();
        assert_eq!(out.op, "clear");
        assert!(items.is_empty());
    }

    #[test]
    fn execute_list_action_out_of_bounds() {
        let mut items: Vec<CloudValue> = Vec::new();
        assert!(execute_list_action(&mut items, &ListAction::DeleteLast).is_none());
        assert!(execute_list_action(&mut items, &ListAction::DeleteAt(3)).is_none());
        assert!(execute_list_action(&mut items, &ListAction::ReplaceAt(0, 1i64.into())).is_none());
        assert!(execute_list_action(&mut items, &ListAction::Insert(2, 1i64.into())).is_none());
    }

    #[test]
    fn parse_cloud_list_operations() {
        assert_eq!(
            parse_list_action(&json!({"action": "append", "value": 1})),
            Some(ListAction::Append(CloudValue::Number(1)))
        );
        assert_eq!(
            parse_list_action(&json!({"action": "insert", "nth": 3, "value": "x"})),
            Some(ListAction::Insert(2, CloudValue::Text("x".into())))
        );
        assert_eq!(
            parse_list_action(&json!({"action": "delete", "nth": "last"})),
            Some(ListAction::DeleteLast)
        );
        assert_eq!(
            parse_list_action(&json!({"action": "delete", "nth": "all"})),
            Some(ListAction::DeleteAll)
        );
        assert_eq!(
            parse_list_action(&json!({"action": "delete", "nth": 1})),
            Some(ListAction::DeleteAt(0))
        );
        assert_eq!(
            parse_list_action(&json!({"action": "replace", "nth": "last", "value": 9})),
            Some(ListAction::ReplaceLast(CloudValue::Number(9)))
        );
        assert!(parse_list_action(&json!({"action": "unknown"})).is_none());
    }

    #[test]
    fn datastore_lookup_by_name_and_cvid() {
        let mut store = DataStore::default();
        store.create_private("1001".into(), "分数".into(), CloudValue::Number(0));
        store.create_list("2001".into(), "名单".into(), vec![]);
        assert!(store.variable(VarKind::Private, "分数").is_some());
        assert!(store.variable(VarKind::Private, "1001").is_some());
        assert!(store.list("名单").is_some());
        assert!(store.list("2001").is_some());
        assert!(store.variable(VarKind::Private, "不存在").is_none());
        store.clear_all();
        assert!(store.variable(VarKind::Private, "1001").is_none());
    }

    #[test]
    fn editor_query_params() {
        assert_eq!(EditorType::Kitten.query_params(), ("1", "1"));
        assert_eq!(EditorType::Nemo.query_params(), ("5", "2"));
        assert_eq!(EditorType::KittenN.query_params(), ("5", "3"));
        assert_eq!(EditorType::Coco.query_params(), ("1", "1"));
    }
    ...
    fn truncate_keeps_both_ends() {
        ...
        assert_eq!(truncate("短文本", 80), "短文本");
    }

    #[test]
    fn cloud_update_writes_back_value() {
        let conn = CloudBuilder::new(1).build();
        {
            let mut store = conn.inner.state.lock().unwrap();
            store.create_private("100".into(), "分数".into(), CloudValue::Number(0));
        }
        UpdatePrivateVarHandler
            .handle(&conn.inner, &json!({"cvid": "100", "value": 99}))
            .unwrap();
        let variable = conn.get_private_variable("分数").unwrap();
        assert_eq!(variable.get(), Some(CloudValue::Number(99)));
    }

    #[test]
    fn cloud_public_update_writes_back_value() {
        let conn = CloudBuilder::new(1).build();
        {
            let mut store = conn.inner.state.lock().unwrap();
    ...
    #[test]
    fn recreate_data_item_keeps_callbacks() {
        let conn = CloudBuilder::new(1).build();
        {
            let mut store = conn.inner.state.lock().unwrap();
            store.create_private("100".into(), "分数".into(), CloudValue::Number(0));
    ...
    #[test]
    fn flush_keeps_commands_when_disconnected() {
        let conn = CloudBuilder::new(1)
        ...
        // 未连接时命令必须保留,等待连接恢复后补发
        assert_eq!(inner.commands.lock().unwrap().len(), 1);
        inner.stopping.store(true, Ordering::Release);
        handle.join().unwrap();
    }

    #[test]
    fn list_change_callback_gets_old_and_new() {
        let conn = CloudBuilder::new(1).build();
        {
            let mut store = conn.inner.state.lock().unwrap();
            store.create_list("L".into(), "名单".into(), vec![CloudValue::Number(1)]);
        }
        let list = conn.get_list("名单").unwrap();
        let captured = std::sync::Arc::new(Mutex::new(Vec::new()));
        let captured2 = captured.clone();
        list.on_change(move |old, new, src| {
            captured2
                .lock()
                .unwrap()
                .push((old.to_vec(), new.to_vec(), src.to_string()));
        });
        list.push(CloudValue::Number(2)).unwrap();
        let got = captured.lock().unwrap().clone();
        assert_eq!(got.len(), 1);
        assert_eq!(got[0].0, vec![CloudValue::Number(1)]);
        assert_eq!(got[0].1, vec![CloudValue::Number(1), CloudValue::Number(2)]);
