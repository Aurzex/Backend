...
mod tests {
    ...
    #[test]
    fn chat_frame_shape() {
        let messages = vec![HistoryMessage::user("你好")];
        let frame = build_chat_frame("abc12345", &messages).unwrap();
        assert!(frame.starts_with("42 [\"chat\","));
        let parsed = parse_frame(&frame);
        match parsed {
            Frame::Event(name, payload) => {
                assert_eq!(name, "chat");
                assert_eq!(payload["session_id"], "abc12345");
                assert_eq!(payload["chat_type"], "chat_v3");
                assert_eq!(payload["messages"][0]["role"], "user");
                assert_eq!(payload["messages"][0]["content"], "你好");
            }
            other => panic!("帧解析失败: {other:?}"),
        }
    }

    #[test]
    fn parse_chat_ack_stream_events() {
        assert_eq!(
            parse_chat_ack(&json!({"code": 1, "data": {"content_type": "stream_output_begin"}})),
        ...
        // 非成功码 / 未知类型返回 None
        assert_eq!(
            parse_chat_ack(&json!({"code": 0, "data": {"content_type": "stream_output_begin"}})),
            None
        );
        assert_eq!(
            parse_chat_ack(&json!({"code": 1, "data": {"content_type": "other"}})),
            None
        );
    }
    ...
    fn parse_frame_variants() {
        ...
        assert_eq!(
            parse_frame("0{\"sid\":\"x\"}"),
            Frame::Handshake(json!({"sid": "x"}))
        );
        assert_eq!(
            parse_frame("42[\"join\"]"),
            Frame::Event("join".into(), Value::Null)
        );
        assert_eq!(parse_frame("garbage"), Frame::Unknown("garbage".into()));
    }
    ...
    #[test]
    fn callbacks_add_remove_emit() {
        let client = ChatBuilder::new("test-token").build();
        let calls = std::sync::Arc::new(std::sync::Mutex::new(Vec::new()));
        let calls2 = calls.clone();
        let handle = client.add_stream_callback(move |content, event| {
            calls2.lock().unwrap().push((content.to_string(), event));
        });
        let inner = &client.inner;
        emit_stream(inner, "hi", ChatEventType::Text);
        client.remove_stream_callback(handle);
        emit_stream(inner, "ignored", ChatEventType::Text);
        let calls = calls.lock().unwrap();
        assert_eq!(calls.len(), 1);
        assert_eq!(calls[0], ("hi".to_string(), ChatEventType::Text));
    }
    ...
    fn send_message_not_connected() {
        let client = ChatBuilder::new("test-token").build();
        assert!(matches!(
            client.send_message("你好", true),
            Err(ChatError::NotConnected)
        ));
        assert!(matches!(
            client.send_and_wait("你好", true, Duration::from_secs(1)),
            Err(ChatError::NotConnected)
        ));
    }
    ...
    #[test]
    fn wait_response_start_after_fast_reply() {
        let client = ChatBuilder::new("test-token").build();
        let inner = &client.inner;
        // 模拟一次消息已发送(回合编号递增)且 AI 快速完成了整轮回复
        inner.pending_round.store(3, Ordering::Release);
        inner.notify.notify_with(|| {
            inner.completed_round.store(3, Ordering::Release);
        });
        assert!(client.wait_for_response_start(Duration::from_millis(50)));
    }

    #[test]
    fn wait_response_start_when_never_sent() {
        let client = ChatBuilder::new("test-token").build();
        // 从未发送消息:应等待真正开始(此处直接超时)
        assert!(!client.wait_for_response_start(Duration::from_millis(30)));
    }

    #[test]
    fn stream_end_writes_history_before_emit() {
        let client = ChatBuilder::new("test-token").build();
        let inner = &client.inner;
        *inner.current_response.lock().unwrap() = "完整回复".to_string();
        ChatAckHandler.handle(
            inner,
            &json!({"code": 1, "data": {"content_type": "stream_output_end", "content": ""}}),
        );
        // 历史先落库
        assert_eq!(client.conversation_history().len(), 1);
        assert_eq!(client.conversation_history()[0].content, "完整回复");
        // 接收状态复位
